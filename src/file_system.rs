...
impl FileSystem {
    ...
    /// Gets the page with the given ID and, if not found in the buffer pool,
    /// adds the page and following pages in the same file to the buffer pool.
    ///
    /// The number of additional pages fetched is determined by
    /// the `self.readahead_buffering` configuration option.
    /// It is limited by the size of the file,
    /// which should be provided as a number of pages via `file_size`.
    ///
    /// Returns a reference to the bytes of the initial page, or an error.
    pub fn get_sequential(
        ...
        let page_end = (page_start + 1 + self.readahead_buffering).min(file_size);
    ...
    fn get_range(
        ...
        #[cfg(false)] // Replace with #[cfg(true)] for additional test output
