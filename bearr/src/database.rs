...
impl Database {
    ...
    pub async fn create(
        ...
        Self::new(name, configuration, metadata).await
    }
    ...
    pub async fn open(name: impl AsRef<Path>) -> Result<Self, DbError> {
        ...
        let mut db = Self::new(name, configuration, metadata).await?;
    ...
    pub async fn get(&self, key: u64) -> Result<Option<u64>, DbError> {
        self.lsm.get(key, &self.file_system).await
    }
    ...
    pub async fn put(&mut self, key: u64, value: u64) -> Result<(), DbError> {
        ...
        let sst_flushed = self.lsm.put(key, value, &self.file_system).await?;
    ...
    pub async fn delete(&mut self, key: u64) -> Result<(), DbError> {
        ...
        let sst_flushed = self.lsm.delete(key, &self.file_system).await?;
    ...
    pub async fn scan(
        ...
        self.lsm.scan(range, &self.file_system).await
    }
    ...
    pub async fn flush(&mut self) -> Result<(), DbError> {
        ...
        self.lsm.flush_memtable(&self.file_system).await?;
    ...
    async fn replay_wal(&mut self) -> Result<(), DbError> {
        assert!(self.wal_enabled);
        ...
        for line in reader.lines() {
            ...
            if parts.len() == 2 {
                ...
                self.lsm.put(key, value, &self.file_system).await?;
            }
        }
