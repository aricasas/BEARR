...
impl Sst {
    ...
    pub async fn open(file_id: FileId, file_system: &FileSystem) -> Result<Sst, DbError> {
        let (btree_metadata, filter) = BTree::open(file_id, file_system).await?;
    ...
    pub async fn get(&self, key: u64, file_system: &FileSystem) -> Result<Option<u64>, DbError> {
        ...
        BTree::get(self, key, file_system).await
    }
    ...
    pub async fn destroy(self, file_system: &FileSystem) -> Result<(), DbError> {
        file_system.delete_file(self.file_id).await?;
        Ok(())
    }
    ...
    pub async fn rename(
        ...
        file_system.rename_file(self.file_id, new_file_id).await?;
