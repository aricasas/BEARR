...
pub struct Eviction {
    ...
    map_out: HashMap<BufferPageId, EntryId>,
    k_in: usize,
    k_out: usize,
}
...
impl Eviction {
    ...
    pub fn new(capacity: usize) -> Result<Self, DbError> {
        ...
        let map_out = HashMap::with_capacity(k_out);
        ...
        match victim {
            EvictionId::AIn(id) => {
                ...
                if self.a_out.len() >= self.k_out {
                    ...
                    self.map_out.remove(&evicted_a_out);
                }
    ...
    pub fn insert_new(&mut self, page_id: BufferPageId) -> EvictionId {
        if let Some(&idx_out) = self.map_out.get(&page_id) {
            ...
            self.map_out.remove(&page_id);
            let id = self.a_m.push_back(page_id);
            EvictionId::AM(id)
        } else {
            let id = self.a_in.push_back(page_id);
            EvictionId::AIn(id)
        }
    }
...
mod tests {
    ...
    use crate::buffer_pool::BufferFileId;
