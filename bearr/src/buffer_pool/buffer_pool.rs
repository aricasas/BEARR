...
#[repr(C)]
#[derive(bytemuck::Pod, bytemuck::Zeroable, Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub struct FileId {
    pub lsm_level: usize,
...
pub struct FileSystem {
    inner: Mutex<InnerFs>,
    directory_fd: io_uring::types::Fd,
    prefix: PathBuf,
    capacity: usize,
...
struct InnerFs {
    buffer_pool: HashMap<BufferPageId, BufferPoolEntry>,
...
impl FileSystem {
    ...
    pub async fn new(
        ...
        let buffer_pool = HashMap::with_capacity(capacity);
    ...
    pub async fn get(&self, page_id: PageId) -> Result<Arc<Aligned>, DbError> {
        let PageId {
            file_id,
            page_number,
        } = page_id;
        self.get_range(file_id, page_number..page_number + 1).await
    }
    ...
    pub async fn get_sequential(
        ...
        self.get_range(file_id, page_start..page_end).await
    }
        ...
        loop {
            ...
            if buffer_data_end > 0 {
                ...
                let bytes_written = unsafe {
                    write(file, offset as u64, bytes.len() as u32, bytes.as_ptr()).await?
                };
                assert_eq!(bytes_written, bytes.len()); // TODO: write rest of unwritten bytes
    ...
    pub async fn delete_file(&self, file_id: FileId) -> Result<(), DbError> {
        let path = self.path(file_id);
    ...
    pub async fn rename_file(
        ...
        unsafe {
            rename(
                self.directory_fd,
                Path::new(&old_file_id.name()),
                self.directory_fd,
                Path::new(&new_file_id.name()),
            )
            .await?
        };
